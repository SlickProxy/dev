namespace SlickProxyLib
{
    ...
    public static class SlickProxyEngine
    {
                ...
                new Func<Func<IDictionary<string, object>, Task>, Func<IDictionary<string, object>, Task>>(
                    next => async env =>
                    {
                        ...
                        try
                        {
                            ...
                            {
                                requestInfo.Settings.OnRedirectTo?.Invoke(from, requestInfo.ProxyObject.RedirectToString);
                                context.Response.Redirect(requestInfo.ProxyObject.RedirectToString);
                            }
                            else if (requestInfo.ProxyObject.ReturnString != null)
                            {
                                requestInfo.Settings.OnRedirectTo?.Invoke(from, requestInfo.ProxyObject.ReturnString);
                            ...
                            else if (requestInfo.ProxyObject.ReturnObjectAsJson != null)
                            {
                                ...
                                requestInfo.Settings.OnRedirectTo?.Invoke(from, json);
                            ...
                            else if (!string.IsNullOrEmpty(requestInfo.RewriteToUrl))
                            {
                                ...
                                requestInfo.Settings.OnRewritingStarted?.Invoke(from, requestInfo.RewriteToUrl, rewriteToSameServer);
                                var myUri = new Uri(requestInfo.RewriteToUrl);

                                if (rewriteToSameServer && !requestInfo.Settings.RouteSameServerRewritesOverNetwork)
                                {
                                    requestInfo.Settings.OnRewriteToCurrentServer?.Invoke(from, requestInfo.RewriteToUrl);
                                    string newPath = myUri.PathAndQuery;
                                    context.Request.Path = new PathString(newPath);
                                    requestInfo.Settings.OnProcessingEnded?.Invoke(from, requestInfo.RewriteToUrl);
                                    await next.Invoke(env);
                                }
                                else
                                {
                                    ...
                                    requestInfo.Settings.OnRewriteToDifferentServer?.Invoke(from, requestInfo.RewriteToUrl);

                                    HttpResponseMessage response = await request.SendAsync(from, requestInfo.RewriteToUrl, requestInfo.Settings.OnRewritingException, requestInfo.Settings.OnRespondingFromRemoteServer);
                                    context.Response.StatusCode = (int)response.StatusCode;
                        ...
                        {
                            requestInfo.Settings.OnRewritingException?.Invoke(from, requestInfo.RewriteToUrl, request ?? OwinRequestToHttpRequestMessage(requestInfo), e);
                            throw;
                        }
                    }));
            return app;
        ...
        public static async Task<HttpResponseMessage> SendAsync(this HttpRequestMessage request, string from, string remote, Action<string, string, HttpRequestMessage, Exception> requestInfoOnRewritingException, Action<string, string, HttpRequestMessage, HttpResponseMessage, Exception, string> requestInfoOnRespondingFromRemoteServer)
        {
            try
            {
                ...
                requestInfoOnRespondingFromRemoteServer?.Invoke(from, remote, request, response, null, "Request succeeded");
            ...
            catch (HttpRequestException e)
            {
                ...
                requestInfoOnRespondingFromRemoteServer?.Invoke(from, remote, request, null, e, "Request failed");
            ...
            catch (PlatformNotSupportedException e)
            {
                ...
                requestInfoOnRespondingFromRemoteServer?.Invoke(from, remote, request, null, e, "Sorry, your system does not support the requested feature.");
                return new HttpResponseMessage
                {
                    StatusCode = 0,
            ...
            {
                requestInfoOnRespondingFromRemoteServer?.Invoke(from, remote, request, null, e, " The request timed out, the endpoint might be unreachable.");
            ...
            catch (Exception ex)
            {
                ...
                requestInfoOnRespondingFromRemoteServer?.Invoke(from, remote, request, response, ex, "Request failed");
                return response;
            }
        }
